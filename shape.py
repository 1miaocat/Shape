...
class Circle(Shape):
    def __init__(self, color, filled, radius: float = 1.0):
...
class Rectangle(Shape):
    def __init__(self, color: str = 'red', filled: bool = True, width: float = 1.0, length: float = 1.0):
        super().__init__(color, filled)
        self.width = width
        self.length = length
    ...
    def to_string(self):
        return f'''This {self.color} rectangle has a width and length of {self.width} and {self.length} respectively.
        Area of the rectangle is {print(self.get_area())} and its perimeter is {print(self.get_perimeter())}.'''
